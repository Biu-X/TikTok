...
func GetUserResponseByBothID(id int64, userID int64) (*UserResponse, error) {
...
func GetUserResponseByID(userID int64) (*UserResponse, error) {
	...
	// 求用户关注了多少个用户，即求表中关注者 ID 为 userId 的列数
	followCount, err := dao.GetFollowingCountByUserID(userID)
	if err != nil {
		return nil, err
	}

	// 求用户的关注者数量，即求表中用户 id 等于 userId 的列数
	followerCount, err := dao.GetFollowerCountByUserID(userID)
	if err != nil {
		return nil, err
	}

	// 作品获赞数量（需要去 Video 表中查询该用户所有的 Video_ID，然后再去 Favorite 表中查询每一个 Video_ID 的获赞数）
	videoIDs, err := dao.GetVideoIDByAuthorID(userID)
	if err != nil {
		return nil, err
	}
	...
	// 总的喜欢作品量
	totalFavorite, err := dao.GetFavoriteCountByUserID(userID)
	if err != nil {
		return nil, err
	}
